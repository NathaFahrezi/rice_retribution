    ...
    <div class="flex-1 overflow-x-hidden overflow-y-auto p-6 md:p-8 scrollbar-hide">
        <div class="bg-white rounded-xl overflow-x-hidden overflow-y-auto shadow-lg w-full p-8 md:p-10">
            ...
            <div class="space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" id="nrp" name="nrp"
                            class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500"
                            value="{{ $profile->nrp }}" placeholder="Contoh: 12345678" readonly required>
                        @error('nrp')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="satker" class="block text-sm font-medium text-gray-700">Satuan Kerja</label>
                        <input type="text" id="satker" name="satker"
                            class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500"
                            value="{{ $profile->polsek->nama ?? false
                                ? ($profile->polres->nama ?? '') . ' - ' . $profile->polsek->nama
                                : $profile->polres->nama ?? '' }}"
                            readonly>
                    </div>
                    <div>
                        ...
                        <input type="text" id="pangkat" name="pangkat"
                            class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500"
                            value="{{ $profile->pangkat }}" placeholder="Contoh: Letkol" readonly required>
                        @error('pangkat')
                            <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <input type="text" id="jabatan" name="jabatan"
                        class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-lime-500 focus:border-lime-500"
                        value="{{ $profile->jabatan }}" placeholder="Contoh: Kepala Bagian" readonly required>
                    @error('jabatan')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <p class="mt-2 text-sm font-medium text-gray-600">Foto KTP:</p>
                        <img src="{{ asset('uploads/ktp/' . $profile->foto_ktp) }}" alt="Foto KTP"
                            class="w-full h-auto rounded shadow mt-1">
                    </div>
                    <div>
                        <p class="mt-2 text-sm font-medium text-gray-600">Foto Wajah:</p>
                        <img src="{{ asset('uploads/wajah/' . $profile->foto_wajah) }}" alt="Foto Wajah"
                            class="w-full h-auto rounded shadow mt-1">
                    </div>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-xl  overflow-x-hidden overflow-y-auto shadow-lg mt-9  w-full p-8 md:p-10">

            <div class="mb-8 text-center md:text-left">
                <h1 class="text-3xl font-bold text-gray-800">Ubah Password</h1>
            </div>

            <form action="{{ route('user.ubah.password') }}" class="space-y-6" method="post">
                @csrf
                <div class="relative">
