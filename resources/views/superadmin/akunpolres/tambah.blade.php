...
@section('title', 'Tambah Akun Polres')
    ...
    <div class="flex-1 overflow-x-hidden overflow-y-auto p-6 md:p-8 scrollbar-hide">
        ...
        <div class="bg-white rounded-xl  overflow-x-hidden overflow-y-auto shadow-lg  w-full p-8 md:p-10">
            ...
            <div class="mb-8 text-center md:text-left">
                <h1 class="text-3xl font-bold text-gray-800">Tambah Akun Polres/ta</h1>
            </div>

            <form action="{{ route('superadmin.tambah.akun.polres.process') }}" class="space-y-6" method="POST">
                @csrf
                <div class="relative">
                ...
                <div>
                    ...
                    <input type="text" id="name" name="name"
                        class="mt-1 block w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm 
                               focus:outline-none focus:ring-2 focus:ring-lime-500 focus:border-lime-500"
                        placeholder="Contoh: restapadang" required>
                    @error('name')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
