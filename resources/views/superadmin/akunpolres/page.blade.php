    ...
    <div class="flex-1 overflow-x-hidden overflow-y-auto p-6 md:p-8 scrollbar-hide">
        <div class="bg-white rounded-xl shadow-md p-6">
            ...
            <div class="flex justify-between items-center mb-4">
                <h3 class="text-lg font-bold text-gray-800">Akun Polres/ta</h3>
                <a href="{{ route('superadmin.tambah.akun.polres') }}"
                        ...
                        @forelse ($akun as $i => $m)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    {{ $m->profile->polres->nama ?? 'N/A' }}
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('superadmin.edit.akun.polres', $m->id) }}"
                                    ...
                                    <form id="delete-form-{{ $m->id }}"
                                        action="{{ route('superadmin.hapus.akun.polres', $m->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
