    ...
    <div class="flex-1 overflow-x-hidden overflow-y-auto p-6 md:p-8 scrollbar-hide">
        ...
        <div class="bg-white rounded-xl  overflow-x-hidden overflow-y-auto shadow-lg  w-full p-8 md:p-10">
            ...
            <div class="mb-8 text-center md:text-left">
                <h1 class="text-3xl font-bold text-gray-800">Edit Data Polres/ta</h1>
            </div>

            <form action="{{ route('superadmin.edit.polres.process', $polres->id) }}" class="space-y-6" method="post">
                @csrf
                @method('PUT')
                <div><label for="nama" class="block text-sm font-medium text-gray-700">Nama
                    </label><input type="text" id="nama" name="nama"
                        ...
                        placeholder="Contoh: Polda Sumatera Barat" value="{{ $polres->nama }}" required>
                    @error('nama')
