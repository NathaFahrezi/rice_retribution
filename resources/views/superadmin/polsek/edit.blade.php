    ...
    <div class="flex-1 overflow-x-hidden overflow-y-auto p-6 md:p-8 scrollbar-hide">
        ...
        <div class="bg-white rounded-xl  overflow-x-hidden overflow-y-auto shadow-lg  w-full p-8 md:p-10">
            ...
            <div class="mb-8 text-center md:text-left">
                <h1 class="text-3xl font-bold text-gray-800">Edit Data Polsek</h1>
            </div>

            <form action="{{ route('superadmin.edit.polsek.process', $polsek->id) }}" class="space-y-6" method="post">
                @csrf
                @method('PUT')
                <div class="relative">
                    ...
                    <select name="polres_id" id="polres-select" class="sr-only" required>
                        ...
                        @foreach ($polres as $i => $m)
                            <option value="{{ $m->id }}" {{ $m->id == $polsek->polres_id ? 'selected' : '' }}>
                                {{ $m->nama }}
                            </option>
                        @endforeach
                ...
                <div>
                    ...
                    <input type="text" id="nama" name="nama"
                        ...
                        placeholder="Contoh: Polsek Padang Timur" value="{{ $polsek->nama }}" required>
                    @error('nama')
                        <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                    @enderror
                </div>
